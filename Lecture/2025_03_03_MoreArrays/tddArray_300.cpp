...
void testRemoveLargest( )
{
    ...
    // { 18, 5, 10, 1300, 2, 7 } BECOMES { 18, 5, 10, 2, 7 }
    int arr1[6] =                   { 18, 5, 10, 1300, 2, 7 };
    int arr1WithLargestRemoved[6] = { 18, 5, 10, 2, 7 }; // there is room for another data point
    // NOTE: arr1 is "pass by reference"
    // the removeLargest function will actually remove it from arr1
    removeLargest( arr1, 6 );
    // not that easy, C++ mean
    // assert( arr1 == arr1WithLargestRemoved );
    assert( isEquivalentIntArr( arr1, arr1WithLargestRemoved, 5 ) );

    int idk[6] = { 5, 10, 2, 7 };
    removeLargest( arr1, 5 );
    assert( isEquivalentIntArr( arr1, idk, 4 ) );
...
int removeLargest( int arr[] , int used )
{
    printIntArray("begin", arr, used);
    // get the index of largest value (can write ourselves, but won't today)
    // int indexOfLargestValue = getIndexOfLargestValue( arr, used );
    int x = getIndexOfLargestValue( arr, used );

    // // REPEAT: for all things to the right of that inde
    // // using `used` vs `used - 1` get different behavior
    // // Ed and Coderunner will sometimes  be super mad and give a vague error
    // // if you forget this -1
    ...
    // return the new used space of the array
    return used - 1;
}
